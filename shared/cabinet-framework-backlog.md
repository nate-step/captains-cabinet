...
- **Status:** SHIPPED 2026-04-22 — Phase A target-correlation regex landed in commit `b6c7cf2`, HOTFIX `37888dc` (COO + Sonnet adversary: 7 additional bugs folded). EVAL-018 extended to 35 positive / 25 negative / CTO bypass = 61 checks, all PASS.
...
- **Phase B scope gaps:** filed as FW-040 (now 9+ gap classes: original 7 + COO's deferred quoted-dest-with-space + COO's install + other write tools `awk`/`dd`/`touch`/`mkdir`/`truncate`/`sqlite3` + `python3 -c` + `node -e`). Deferred pending Component selection (shell-parser vs allow-list vs fs-watcher audit vs incremental regex).
- **Realized effort:** M (~4h total — Phase A v1 + two adversary passes + hotfix + EVAL-018 extension from 22 → 61 checks + rsync `-rt` false-pos guard).
