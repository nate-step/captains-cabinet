...
- **Status:** Phase A shipped 2026-04-21 (commit `15b94f8` — 4 fixes + COO ACK). EVAL-009 + EVAL-010 shipped 2026-04-21 (this commit). Phase B L-findings + M-4 edge-cases still proposed.
- **Phase A — SHIPPED:**
...
- **Phase B — Proposed (deferred L findings + regression evals):**
  1. **L-6: `date -d "$LAST_CALL"` fallback always-fires idle-warning on malformed value.** `post-tool-use.sh:~337` — `|| echo "0"` means a corrupted Redis value (old format, partial write) makes `IDLE_SECONDS` huge, permanent idle warnings. Fix: validate `LAST_CALL` matches ISO-8601 regex before epoch conversion. Low severity (requires corruption).
  2. **L-7: `CAPTAIN_CHAT_ID` silently empty → decision enforcement permanently disabled.** `post-tool-use.sh:~318` — `grep ... platform.yml` returns empty if key drifts or file missing; line 323's `[ -n "$CAPTAIN_CHAT_ID" ]` guard silently suppresses the decision reminder with no diagnostic. Fix: emit `WARN: captain_telegram_chat_id not resolved` to stderr when empty AND officer has `logs_captain_decisions` capability.
  ...
  4. **M-4 edge-cases (COO review of commit `15b94f8`, 2026-04-21 21:05 UTC — non-blocking):**
     - **(a) `git push main` single-space no-origin form no longer matches** — new regex requires content between `push` and the refspec so a single space can't satisfy both `[[:space:]]+` and trailing `[[:space:]]`. Low-prob regression: `git push main` without a remote is not a valid git invocation (main interpreted as remote name, not refspec), so the "miss" is arguably correct. Double-space variant backtracks correctly.
     - **(b) `git push --dry-run origin main` now triggers AUTO-DEPLOY (FP)** — `--dry-run` satisfies the `.*` slot, so validators get a notification for a dry-run push. Blast radius: validators poll Vercel, find no new deploy, report back; minor noise. Fix ideas: negative-lookahead on `--dry-run` (hard in ERE), or swap `.*` for `([^[:space:]]+[[:space:]]+)*` to anchor on non-whitespace tokens and add explicit `--dry-run` exclusion before the regex match.
  5. **Rejected false-positive (documented for future triage):** Audit Finding #5 claimed `CALL_COUNT` modulo "floods every call when Redis down." Trace: line 303 defaults `CALL_COUNT=1` on Redis failure; `1 % 50 = 1 ≠ 0`; modulo guard on lines 306/358 is FALSE — nudges never fire during Redis outage (opposite of claim, which is actually graceful-degrade behavior). Rejected per `feedback_adversary_scope_gap_vs_bug.md` with control-flow citation.
- **Effort:** Phase B remaining = XS (~0.5h — L-6 + L-7 + M-4 edge cases).
- **Owner:** CTO.
