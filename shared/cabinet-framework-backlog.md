...
- **ACs (day-1):**
  ...
  - AC-5: Write-path retargeting lands post-bulk-pass (not concurrent). The post-tool-use MCP-tool-call branch detects `mcp__library__library_update_record` and fires the same experience-nudge / captain-decision / cross-validation branches that the Write-tool branch already fires for `shared/interfaces/product-specs/`, `shared/interfaces/research-briefs/`, `shared/interfaces/deployment-status/` — i.e., Spec 037 retargeting must NOT lose FW-033 Write-branch detector coverage. Pin as EVAL case: MCP library_update_record against a pre-migration-tracked path fires the same nudge as the pre-migration Write tool call.
  - AC-6: In-flight quarantine: during the bulk pass, any `library_update_record` MCP call blocks on the advisory lock OR hook retargeting is disabled (toggle via env var `LIBRARY_MIGRATION_IN_PROGRESS=1`). Importer sets the env + advisory lock + clears on completion.
  - AC-7: Rollback path — migration-abort script deletes `kb_records` / `library_record_links` / `library_record_sections` rows written during the current pass (keyed on `created_at >= <migration_start_ts>`), releases advisory lock, clears `LIBRARY_MIGRATION_IN_PROGRESS`. Repo files untouched by design (importer is READ-ONLY on repo side).
