...
- **Phase C — Partial SHIPPED, remainders proposed:**
  1. **LAST_EXPERIENCE symmetric L-6 port — SHIPPED (this commit).** `post-tool-use.sh:~412` — same ISO-8601 shape guard + WARN pattern as L-6, ported to the `cabinet:last-experience:$OFFICER` branch. Adversary-identified symmetric site.
  2. **EVAL-012 hardening per COO observations on `7f719b5` — SHIPPED (this commit).**
  ...
  3. **HEAD:main deploy-regex scope-gap — proposed.** `git push origin HEAD:main` has no space before `main`, so `(.*[[:space:]])?(main|master)([[:space:]]|$)` can't match. Low frequency. Fix: extend to `[[:space:]/:]` class or add a second elif for the `:ref` form.
  4. **`;`-separator scope-gap — proposed (COO observation on `bde229e`).** `git push origin main; echo done` — trailing `;` isn't space, so `([[:space:]]|$)` terminator misses. Parallel to HEAD:main; fix together. Low frequency.
  5. **EVAL-011 split-range ordering — proposed (COO observation on `bde229e`).** `EV11_DRYRUN_LINE/DEPLOY_LINE` both use `head -1` → only block-5 ordering asserted. If future refactor desyncs block-6 (deploy-elif moved above dry-run-elif), block-5 still passes. Fix: assert ordering within a split range per block.
  6. **L-6 semantic validation — noted (COO observation #3 on `7f719b5`).** Shape-only regex accepts `2026-13-45T25:70:99Z`; `date -d` returns 0 and flood persists. Unreachable today (only writer is `date -u` which cannot emit invalid values); if a non-`date -u` writer ever appears, add `date -d "$value" >/dev/null 2>&1` as a secondary gate. Lower priority than HEAD:main / `;`-separator.
- **Effort:** Phase C remaining = XS (~0.2h — 2 regex scope extensions + EVAL-011 split-range; C-6 lower priority).
- **Owner:** CTO.
