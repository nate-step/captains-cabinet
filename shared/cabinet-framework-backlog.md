...
- **Status:** Phase A shipped 2026-04-21 (commit `15b94f8` — 4 fixes + COO ACK). EVAL-009/010 shipped 2026-04-21 (commit `0cd5129`). M-4 edge-cases + EVAL-011 shipped 2026-04-21 (this commit). Phase B L-findings (L-6 + L-7) + HEAD:main scope-gap still proposed.
- **Phase A — SHIPPED:**
...
- **Phase B — Proposed (deferred L findings + regression evals):**
  ...
  4. **M-4 edge-cases — SHIPPED 2026-04-21 (this commit):**
     ...
     - **Scope-gap: `git push origin HEAD:main`** — refspec with `:` has no space before `main`, so `(.*[[:space:]])?` can't satisfy; deploy form silently missed. Low-frequency (most officers push bare `main` or via tokenized URL). Deferred.
...
- **Effort:** Phase B remaining = XS (~0.3h — L-6 + L-7 + HEAD:main refspec scope-gap).
- **Owner:** CTO.
