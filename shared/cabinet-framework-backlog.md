...
- **Status:** Phase A shipped 2026-04-21 (commit `15b94f8` — 4 fixes + COO ACK). EVAL-009/010 shipped 2026-04-21 (commit `0cd5129`). M-4 edge-cases + EVAL-011 shipped 2026-04-21 (commit `bde229e`). L-6 + L-7 + EVAL-012 shipped 2026-04-21 (this commit). Phase C remainders: HEAD:main + `;`-separator deploy-regex scope-gaps, LAST_EXPERIENCE symmetric L-6 bug, EVAL-011 split-range ordering check.
- **Phase A — SHIPPED:**
...
- **Phase C — Proposed (remainders):**
  1. **HEAD:main deploy-regex scope-gap.** `git push origin HEAD:main` has no space before `main`, so `(.*[[:space:]])?(main|master)([[:space:]]|$)` can't match. Low frequency. Fix: extend to `[[:space:]/:]` class or add a second elif for the `:ref` form.
  2. **`;`-separator scope-gap (COO observation on `bde229e`).** `git push origin main; echo done` — trailing `;` isn't space, so `([[:space:]]|$)` terminator misses. Parallel to HEAD:main; fix together. Low frequency.
  3. **EVAL-011 split-range ordering (COO observation on `bde229e`).** `EV11_DRYRUN_LINE/DEPLOY_LINE` both use `head -1` → only block-5 ordering asserted. If future refactor desyncs block-6 (deploy-elif moved above dry-run-elif), block-5 still passes. Fix: assert ordering within a split range per block.
  4. **LAST_EXPERIENCE symmetric L-6 bug (Sonnet adversary on this commit).** `post-tool-use.sh:~407` — `date -d "$LAST_EXPERIENCE" +%s 2>/dev/null || echo "0"` has the same flood-on-corruption class as L-6 (records SINCE_LAST_RECORD = NOW_EPOCH, fires the "PROACTIVE WORK CHECK" every 50th call for 24h). Apply same ISO-8601 shape guard + WARN. Extend EVAL-012 to cover both sites.
- **Effort:** Phase C = XS (~0.3h — 3 regex/scope extensions + 1 symmetric L-6 port).
- **Owner:** CTO.
