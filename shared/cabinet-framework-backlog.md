...
- **Status:** Phase A shipped 2026-04-21 (commit TBD — 4 fixes). Phase B proposed.
- **Phase A — SHIPPED:**
...
- **Phase B — Proposed (deferred L findings + regression evals):**
  ...
  3. **EVAL coverage:** Two new golden evals needed now that Phase A asserts stderr on failure:
     - EVAL-009: stub `redis-cli` to exit 1; invoke `trigger_send`; assert stderr contains "XADD to cabinet:triggers:$target failed"; assert exit 0 (hook continues).
     - EVAL-010: rename `lib/triggers.sh` out of place; invoke a scenario that sources it; assert stderr contains "triggers.sh failed to load"; restore.
  4. **Rejected false-positive (documented for future triage):** Audit Finding #5 claimed `CALL_COUNT` modulo "floods every call when Redis down." Trace: line 303 defaults `CALL_COUNT=1` on Redis failure; `1 % 50 = 1 ≠ 0`; modulo guard on lines 306/358 is FALSE — nudges never fire during Redis outage (opposite of claim, which is actually graceful-degrade behavior). Rejected per `feedback_adversary_scope_gap_vs_bug.md` with control-flow citation.
- **Effort:** Phase B = S (~1-2h including the two evals).
- **Owner:** CTO.
